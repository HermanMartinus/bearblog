...
def list(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if not request.user.settings.upgraded and blogs.count() >= 1:
                form.add_error('title', 'Upgrade to create more than one blog.')
            elif blogs.count() >= request.user.settings.max_blogs:
    ...
    return render(request, 'studio/blog_list.html', {
        'blogs': blogs,
        'form': form,
        'payment_management_url': settings.payment_management_url,
        'has_ongoing_subscription': settings.upgraded and settings.plan_type in ('monthly', 'yearly'),
    })
