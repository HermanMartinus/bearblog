...
class ProtectedRouteMiddlewareTests(TestCase):
    def _middleware(self):
        from django.http import HttpResponse
        from blogs.middleware import ProtectedRouteMiddleware
        return ProtectedRouteMiddleware(lambda request: HttpResponse('ok'))

    def _get(self, path, host='example.com', cookies=None):
        request = RequestFactory().get(path, HTTP_HOST=host)
        request.COOKIES.update(cookies or {})
        return self._middleware()(request)

    def test_protected_path_with_query_renders_interstitial(self):
        from blogs.middleware import ProtectedRouteMiddleware

        with self.assertTemplateUsed('middleware/protected_route_interstitial.html'):
            response = self._get('/blog/?tag=python')

        self.assertEqual(response.status_code, 403)
        self.assertContains(response, '🐻 Checking your browser', status_code=403)
        self.assertContains(response, '"cookieName": "bear_clearance"', status_code=403)
        self.assertContains(
            response,
            f'"workDurationMs": {ProtectedRouteMiddleware.WORK_DURATION_MS}',
            status_code=403,
        )
        self.assertContains(response, 'window.setTimeout(work, 0)', status_code=403)

    def test_unprotected_requests_pass_through(self):
        for path in ('/blog/', '/somewhere/?tag=python'):
    ...
    def test_valid_cookie_passes_through(self):
        ...
        cookie = signing.dumps(
            {'host': 'example.com'},
            salt=ProtectedRouteMiddleware.COOKIE_SALT,
        )
        response = self._get(
            '/blog/?tag=python',
            cookies={ProtectedRouteMiddleware.COOKIE_NAME: cookie},
        )

        self.assertEqual(response.status_code, 200)

    def test_invalid_or_wrong_host_cookie_is_challenged(self):
        from django.core import signing
        from blogs.middleware import ProtectedRouteMiddleware

        wrong_host_cookie = signing.dumps(
            {'host': 'different.example.com'},
            salt=ProtectedRouteMiddleware.COOKIE_SALT,
        )
        for cookie in ('invalid', wrong_host_cookie):
    ...
    def test_interstitial_is_not_cached_or_indexed(self):
        response = self._get('/blog/?tag=python')
...
class CodeBlockTests(TestCase):
    ...
    def test_python_produces_span_tokens(self):
    ...
    def test_css_from_formatter_matches_highlight_class(self):
...
class HighlightCssTests(TestCase):
    ...
    def test_highlight_css_contains_scoped_selectors(self):
        ...
        with open(css_path) as f:
            css = f.read()
        self.assertIn('.highlight', css)
        self.assertIn('.highlight .k', css)  # Keyword
